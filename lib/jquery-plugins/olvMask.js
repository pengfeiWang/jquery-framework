 ...
 * @Author:        Oliver
 * @prefix:        olv
 * @createTime:    2015-08-11 13:45:34
 * @Description:   遮罩插件, olvMask to $.ui, 唯一
 * @updateTime:    2015-08-27 13:46:33
*/
/* 遮罩 */
/*
 // 显示遮罩
 ...
 // 显示
 $.ui.olvMask.show(ops, callback)
 $.ui.olvMask.show(callback)
 // 隐藏
 olvMask.hide(ops, callback)
 olvMask.hide(callback)
 */
;
(function ( factory ) {
	...
	function ( $ ) {
		...
		Mask.hide = function ( ops, callback ) {
			...
			if ( $.isFunction( ops ) ) {
				newOps.hideCallBack = ops
			} else {
				newOps = $.extend(true, {}, newOps, ops);
				newOps.hideCallBack = callback;
			}
			var obj = createOps(newOps);
