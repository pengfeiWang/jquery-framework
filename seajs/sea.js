...
;(function ( seajs ) {
	// seajs.modUrl = window.location.origin;
	seajs._pros = function () {
		var el = document.getElementById('seajsnode');
		if(!el) return;
		var str =  el.getAttribute('data') ? el.getAttribute('data').replace(/'/g, '"') : '';
		var json = JSON ? JSON.parse(str) : (str ? eval( '(' +str+ ')') : '');
		return {
...
if ( seajs._pros && !!seajs._pros[ "pro" ] ) {
	seajs.use('{purl}');
}